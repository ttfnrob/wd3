...
total = MongoClassification.collection.find({:created_at => { '$gte' => Time.parse(most_recent_date.to_s)}}).count()
puts "#{total} of #{MongoClassification.size} classifications to process. Go!"

counter = 0
MongoClassification.collection.find({:created_at => { '$gte' => Time.parse(most_recent_date.to_s)}}, :sort => ['created_at', Mongo::ASCENDING], :timeout => false) do |cursor|
    cursor.each do |cl|
      	c = MongoClassification.load(cl)
		counter+=1
